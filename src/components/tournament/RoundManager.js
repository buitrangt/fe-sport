...
const RoundManager = ({ tournament, currentRound = 1, onRoundAdvanced }) => {
  ...
  const advanceRoundMutation = useMutation(
    () => tournamentKnockoutService.advanceRound(tournament.id),
    {
      ...
      onError: (error) => {
        console.error('Không thể chuyển vòng:', error);
  ...
  const completeTournamentMutation = useMutation(
    ...
    {
      onSuccess: () => {
        showNotification('success', 'Giải đấu đã hoàn thành! 🎉');
        queryClient.invalidateQueries(['tournaments']);
        queryClient.invalidateQueries(['tournament-bracket']);
      },
      onError: (error) => {
        console.error('Không thể hoàn thành giải đấu:', error);
        showNotification('error', error.response?.data?.message || 'Không thể hoàn thành giải đấu');
      }
    }
  );
  ...
  const handleCompleteTournament = () => {
    if (window.confirm('Bạn có chắc chắn muốn hoàn thành giải đấu này không? Hành động này không thể hoàn tác.')) {
  ...
  try {
    ...
    if (Array.isArray(currentMatches?.data?.matches)) {
      ...
      console.log('🚷 [RoundManager] Không tìm thấy mảng trận đấu hợp lệ trong:', currentMatches);
      matchesList = [];
    }
  } catch (err) {
    console.error('🚨 [RoundManager] Lỗi khi trích xuất dữ liệu trận đấu:', err);
    matchesList = [];
  }
  ...
  // Debug logging
  console.log('🔍 [RoundManager] Thông tin gỡ lỗi:', {
    currentRound,
    tournamentTeams,
    expectedRounds,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">Quản lý tiến trình giải đấu và chuyển vòng</p>
          </div>
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <h4 className="text-lg font-medium text-green-900">Vòng {currentRound} Hoàn thành!</h4>
                  <p className="text-green-700 mt-1">
                    Tất cả các trận đấu trong vòng này đã hoàn thành. Bạn có thể chuyển sang Vòng {currentRound + 1}.
                  </p>
                </div>
                <button
      ...
      {bracketData?.rounds && (
        <div className="card">
          <h4 className="text-lg font-semibold text-gray-900 mb-4">Các Vòng đấu Giải đấu</h4>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md mx-4">
            <div className="text-center space-y-4">
              ...
              <p className="text-gray-600">
                Thao tác này sẽ tạo các trận đấu cho vòng tiếp theo dựa trên những người chiến thắng từ Vòng {currentRound}. 
