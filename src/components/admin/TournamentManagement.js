...
import ConfirmModal from './ConfirmModal'; // Đảm bảo đường dẫn này đúng
...
const TournamentManagement = () => {
    ...
    // === START: State và logic cho ConfirmModal ===
    const [showConfirmModal, setShowConfirmModal] = useState(false);
    const [tournamentToOperate, setTournamentToOperate] = useState(null); // Lưu ID giải đấu cần thao tác
    const [confirmModalType, setConfirmModalType] = useState(''); // 'delete' hoặc 'start'
    // === END: State và logic cho ConfirmModal ===
    ...
    const deleteTournamentMutation = useMutation(
        (tournamentId) => tournamentService.deleteTournament(tournamentId),
        {
            ...
            onError: (error) => {
                ...
                handleCloseConfirmModal(); // Đóng modal ngay cả khi có lỗi
            }
        }
    );
    ...
    // === START: Xử lý mở ConfirmModal cho các hành động ===
    const handleOpenConfirmModal = (tournamentId, type) => {
        setTournamentToOperate(tournamentId);
        setConfirmModalType(type);
        setShowConfirmModal(true);
    };

    const handleConfirmAction = () => {
        if (confirmModalType === 'delete' && tournamentToOperate) {
            deleteTournamentMutation.mutate(tournamentToOperate);
        } else if (confirmModalType === 'start' && tournamentToOperate) {
            startTournamentMutation.mutate(tournamentToOperate);
        }
    };

    const handleCloseConfirmModal = () => {
        setShowConfirmModal(false);
        setTournamentToOperate(null); // Clear ID sau khi đóng
        setConfirmModalType(''); // Clear loại hành động
    };
    // === END: Xử lý mở ConfirmModal cho các hành động ===
    ...
    const getStatusAction = (tournament) => {
        switch (tournament.status) {
            ...
            case 'REGISTRATION_OPEN':
                return (
                    <button
                        onClick={() => handleOpenConfirmModal(tournament.id, 'start')} // Mở ConfirmModal cho hành động bắt đầu
    ...
    // Dynamic props cho ConfirmModal dựa trên confirmModalType
    const getConfirmModalProps = () => {
        if (confirmModalType === 'delete') {
            return {
                title: 'Xác nhận Xóa Giải đấu',
                message: 'Bạn có chắc chắn muốn xóa giải đấu này? Hành động này không thể hoàn tác.',
                confirmText: 'Xóa',
                confirmButtonClass: 'btn-danger',
                icon: 'delete',
                isLoading: deleteTournamentMutation.isLoading
            };
        } else if (confirmModalType === 'start') {
            return {
                title: 'Xác nhận Bắt đầu Giải đấu',
                message: 'Bạn có chắc chắn muốn bắt đầu giải đấu này? Hành động này sẽ thay đổi trạng thái của giải đấu.',
                confirmText: 'Bắt đầu',
                confirmButtonClass: 'btn-primary', // Hoặc một class khác cho hành động bắt đầu nếu bạn có
                icon: 'play',
                isLoading: startTournamentMutation.isLoading
            };
        }
        return {}; // Mặc định trả về rỗng
    };
    ...
    return (
        <div className="space-y-6">
                                    ...
                                    <tr key={tournament.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div className="flex items-center justify-end space-x-2">
                                                ...
                                                <button
                                                    onClick={() => handleOpenConfirmModal(tournament.id, 'delete')} // Mở ConfirmModal cho hành động xóa
            ...
            <ConfirmModal
                ...
                {...getConfirmModalProps()} // Truyền các props động
            />
        </div>
    );
};
